...
- Delete: `mcp_server.py`, `run_server.sh`, `Dockerfile`, `claude_code_config.json`, `ignition_tools_summary.json`, `CLAUDE_CODE_SETUP.md`, `src/ignition_mcp/ignition_client.py`, `src/ignition_mcp/main.py`, `src/ignition_mcp/tools/`, `tests/test_client.py`, `tests/test_tools.py`, `tests/test_integration.py`, `tests/conftest.py`, `tests/__init__.py` (so `from conftest import ...` resolves), `.github/workflows/*docker*` if present
...
git rm -rq mcp_server.py run_server.sh Dockerfile claude_code_config.json ignition_tools_summary.json CLAUDE_CODE_SETUP.md \
  ...
  tests/test_client.py tests/test_tools.py tests/test_integration.py tests/conftest.py tests/__init__.py
...
```
Note: `populate_by_name=True` lets tests and the CLI construct `Settings(ign_bin=...)` by field name. `validation_alias` on a field bypasses `env_prefix`, so `IGN_BIN` and `IGNITION_PROFILE` are read verbatim while `host`/`port` read `IGN_MCP_HOST`/`IGN_MCP_PORT`.
...
```
All six are about the old server. Run:
```bash
git rm docs/api-reference.md docs/configuration.md docs/contributing.md docs/examples.md docs/installation.md docs/troubleshooting.md docs/webdev-setup.md
```
Keep only `docs/superpowers/`. The README now carries install, settings, and contributing.

- [ ] **Step 3: Fix CI**

`.github/workflows/ci.yml` exists (and `release.yml`). Make ci.yml's job steps exactly:
```yaml
...
- run: uv run pytest
```
Remove any Docker build job from `ci.yml` and `release.yml`; if `release.yml` only builds Docker, `git rm` it.
