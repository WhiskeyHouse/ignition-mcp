...
class Settings(BaseSettings):
    ...
    ignition_gateway_url: str = Field(
        default="http://localhost:8088", description="Base URL for the Ignition Gateway"
    )

    ignition_username: str = Field(
        default="admin", description="Username for Ignition Gateway authentication"
    )

    ignition_password: str = Field(
        default="password", description="Password for Ignition Gateway authentication"
    )
