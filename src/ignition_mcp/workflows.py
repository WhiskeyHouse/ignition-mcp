...
def register_workflows(mcp: FastMCP, backend: IgnBackend) -> None:
    ...
    async def diagnose_gateway() -> ToolResult:
        ...
        degraded = any(
            m.get("state") is not None and m.get("state") != "RUNNING" for m in (modules or [])
        ) or any(
            d.get("status") is not None and d.get("status") != "Valid"
            for d in (connections or {}).get("databases", [])
        )
        return r.ok(
